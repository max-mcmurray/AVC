...
int reverseSpeed = 50;
int defaultSec = 0;
int defaultMicroSec  = 2000; 	//2.5ms or 0.0025 seconds 
int reverseMicroSec = 500000;				
int rowToScan = height / 2;
int lowerRowToScan = height - 10;
...
void lineMaze() {
	...
	while (1){
		...
		for (int pixel = 0; pixel < width; pixel++) {
			int pixelValue = get_pixel(0, pixel, 3);
		...
		for (int pixel = 0; pixel < height; pixel++) {
			int pixelValue = get_pixel(pixel, width, 3);
		...
		//For line 2
		for (int pixel = 0; pixel < width; pixel++) {
			int pixelValue = get_pixel(height, pixel, 3);
		...
		//For line 3
		for (int pixel = 0; pixel < height; pixel++) {
			int pixelValue = get_pixel(pixel, 0, 3);
		...
		if (maxs[3] > whiteValue) {
			while (get_pixel(0, height, 3) < whiteValue) {
				goForward(5000);
				set_motor(leftMotor, 0);
				set_motor(rightMotor, 0);
				take_picture();
